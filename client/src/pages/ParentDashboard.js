...
const ParentDashboard = () => {
  ...
  const downloadPDF = () => {
    ...
    const cols = ['Roll Number', 'Subject', 'Exam Type', 'Marks', 'Grade', 'Status'];
  ...
  const getStudentStats = (studentEmail) => {
    const studentResults = results.filter(r => r.student.user.email === studentEmail);
    if (studentResults.length === 0) return null;

    const totalMarks = studentResults.reduce((sum, r) => sum + r.marks, 0);
    const avgMarks = (totalMarks / studentResults.length).toFixed(2);
    const passCount = studentResults.filter(r => r.status === 'pass').length;
    const percentage = ((passCount / studentResults.length) * 100).toFixed(2);

    return { avgMarks, percentage, passCount, total: studentResults.length };
  };

  const uniqueStudents = [...new Set(results.map(r => r.student.user.email))];
    ...
    <>
      ...
      <div className="container">
        <div className="card">
          ...
          <p>Monitor student academic performance</p>
        </div>
        ...
        {uniqueStudents.length > 0 && (
          <div className="card">
            ...
            {uniqueStudents.map(studentEmail => {
              const stats = getStudentStats(studentEmail);
              return (
                <div key={studentEmail} style={{ marginBottom: '1rem', padding: '1rem', borderBottom: '1px solid #ddd' }}>
                  ...
                  {stats && (
                    <>
                      ...
                      <p><strong>Passing Subjects:</strong> {stats.passCount}/{stats.total}</p>
                    </>
                  )}
                </div>
              );
